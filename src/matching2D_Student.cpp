...
// I implemented these functions in a way to return the number of keypoints and the processing time
// This information will be used to creat different performance evaluation (.csv) files.
pair<int, double> detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    // Assigning detector parameters
    int blockSize = 2;
    int apertureSize = 3;
    int minResponse = 100;
    double k = 0.04;

    // Monitoring time performance of Harris Keypoint detection
    double t = (double)cv::getTickCount();

    // Detecting Harris corners and normalizing/scaling the output
    ...
    // Filtering the most important keypoints
...
pair<int, double> detKeypointsModern(std::vector<cv::KeyPoint>& keypoints, cv::Mat& img, std::string detectorType, bool bVis)
{
